...
1. Explain the difference between `RDBMS` and `SQL`.
   RDBMS is the system used to structure the data in your database, and SQL is used to communicate to your database.

1. Why do tables need a `primary key`?
   So there can never be duplicates and we can use primary key to reference the PK in another table as a foreign key.

1. What is the name given to a table column that references the primary key on another table.
   Foreign key

1. What do we need in order to have a _many to many_ relationship between two tables.
   Must use JOIN statements and must refer to multiple foreign keys
